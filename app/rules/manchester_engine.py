...
def _make_decision(
    ...
    return ManchesterDecision(
        classification_status="MTS_CATEGORY_ASSIGNED_PENDING_CLINICIAN_REVIEW",
...
def run_manchester_engine(triage_input: TriageTimeInput) -> ManchesterDecision:
    ...
    Decision flow:
      ...
      4. If no approved ruleset: pathway matched but blocked
         → AWAITING_APPROVED_CLINICAL_RULESET
      5. Missing chief complaint or no pathway match
         → REQUIRES_CLINICIAN_REVIEW
    """
    ruleset = get_approved_ruleset()

    # Step 1 — critical physiology (always active, no MTS category assigned)
    ...
    if concern_flags and result.priority is not None and result.priority >= 3:
        ...
        return ManchesterDecision(
            ...
            priority=priority,
            max_wait_minutes=wait,
            reason_codes=list(result.reason_codes) + ["CONCERN_VITAL_UPGRADE"] + concern_flags,
            requires_clinician_review=True,
